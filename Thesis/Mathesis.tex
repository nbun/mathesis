...
This chapter introduces the languages Curry and Coq as well as the modelling methods that are applied in the next chapters of the thesis.
...
Based on this idea of an operator that allows us to explicitly express sharing, we can now begin modelling call-time choice.
...
Similar to the implementation of \citet{fischer2009purely}, we parametrise all definitions with \hinl{MonadPlus} instead of using a static monad.
...
To model effects like non-determinism, the program type needs to be parametrised over effect functors like \hinl{ND} of type \hinl{* -> *}.
...
This is very useful for modelling effectful programs because it allows us to combine syntax using the \hinl{do} notation.
...
The class is parametrised over two functors, one of which is a subtype regarding \hinl{:+:} of the other.
...
Although \hinl{shareArgs} is parametrized over a function that generalizes the type of the sharing operator, it is used only with \hinl{share} in this model.
...
The goal of this chapter is to transfer the Haskell implementation of call-time choice modelled as effect to Coq.
...
Consequently, Coq rejects \mintinline{Coq}{Free} because it is not guaranteed that all possible instances of \cinl{F} fulfil the strict positivity requirement.
...
However, only strictly positive functors can be modelled as a container.
...
The first-order approach of modelling effects shown in the previous sections allows representing scoped effects only with explicit scope delimiters.
...
This definition is isomorphic to the original data type, but it does not fit the container class, because the shape -- and consequently the position function -- need to be parametrized over the functor \hinl{m} in order to store a value of type \hinl{m x}.
...
Therefore, the shape does not need to be parametrized over \cinl{F}.
...
Furthermore, indexed containers do not require a parametrized shape or \cinl{Pos} function, which was the reason why the first definition of higher-order containers in \autoref{subsec-higherOrderContainers} failed.
