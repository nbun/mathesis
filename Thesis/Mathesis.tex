...
when running \mintinline{Haskell}{sort}. This matches our observation above, 
...
If we consider the implementation of \mintinline{Haskell}{isSorted}, we can see 
that, as soon as the comparison of two elements yields 
\mintinline{Haskell}{False}, the function returns \mintinline{Haskell}{False} 
...
However, since we use bind to pass permutations from \mintinline{Haskell}{perm} 
to \mintinline{Haskell}{isSorted}, each permutation is fully evaluated before 
...
Similarly, when we consider the Curry example \mintinline{Haskell}{head (1 : 
head [] : [])}, the strictness of our \mintinline{Haskell}{MonadPlus} approach 
shows again. The corresponding Haskell expression is as follows.

\mint{Haskell}{hd [] >>= \x -> hd (1 : x : [])}

Here \mintinline{Haskell}{hd :: MonadPlus a => [a] -> m a} is the lifted 
\mintinline{Haskell}{head} function. Evaluating the expression in Haskell 
yields \mintinline{Haskell}{mzero}, that is, no result, while Curry returns 
\mintinline{Haskell}{1}. The reason is the definition of the bind operator. For 
example, the monad instance for lists defines bind as \mintinline{Haskell}{xs 
>>= f = concatMap f xs}. In the expression above, this means that the pattern 
matching within \mintinline{Haskell}{concatMap} evaluates 
\mintinline{Haskell}{hd []} to \mintinline{Haskell}{mzero} and thus returns 
\mintinline{Haskell}{mzero}.
...
\mintinline{Haskell}{cons} and \mintinline{Haskell}{nil} make handling the new 
...
replace \mintinline{Haskell}{[]} with \mintinline{Haskell}{List m}. However, 
this is not sufficient because the list itself can be the result of a 
non-deterministic computation. Therefore, an additional \mintinline{Haskell}{m} 
is wrapped around every occurrence of \mintinline{Haskell}{List}.
...
to extract a \mintinline{Haskell}{List} value. Since this only evaluates flat 
...
definition of \mintinline{Haskell}{sort} does not need bind in order to pass 
permutations to \mintinline{Haskell}{isSorted}.
...
We are now able to take advantage of \mintinline{Haskell}{isSorted}'s non-strict
...
\mintinline{Haskell}{perm} that are demanded by \mintinline{Haskell}{isSorted} 
are executed.
...
\mintinline{Haskell}{share} takes a  computation and then returns a computation 
that returns the result, that is, the shared value. The reason for this nesting 
of monad layers is that, in short, the \mintinline{Haskell}{share} combinator 
...
With the explicit sharing operator we can adapt \mintinline{Haskell}{perm'} to
...
The \mintinline{Haskell}{share} operator must satisfy certain laws, which we 
discuss in \autoref{LOS}. The implementation of \mintinline{Haskell}{share} is 
...
Haskell. We still use \mintinline{Haskell}{MonadPlus} to parameterize our 
...
Here, \mintinline{Haskell}{Void} means the absence of effects. 
...
constructor of our new program data type should be \mintinline{Haskell}{Return 
a} for the type \mintinline{Haskell}{a}, that is, the result type of the 
program. To model effects like non-determinism, the program is parameterized 
over effect functors of type \mintinline{Haskell}{* -> *} that represent, for 
example, \mintinline{Haskell}{Fail} and \mintinline{Haskell}{Choice}. We call 
this argument \mintinline{Haskell}{sig} because the signature of a program 
tells us which effects can occur. So far, programs are defined as 
\mintinline{Haskell}{data Prog sig a = Return a}. In order do make use of the 
\mintinline{Haskell}{sig} component, we need to add a constructor for impure 
operations. The \mintinline{Haskell}{ND} data type shows us that effect 
...
With this definition of \mintinline{Haskell}{Prog}, we are able to represent 
the original functors by instantiating \mintinline{Haskell}{sig} appropriately. 
For \mintinline{Haskell}{Void}, we already have the 
\mintinline{Haskell}{Return} constructor. Therefore, the data type we can use 
with \mintinline{Haskell}{Prog} does not need a constructor anymore, that is, 
\mintinline{Haskell}{data Void' a}. 
...
The type \mintinline{Haskell}{Prog Void'} now resembles the original type 
\mintinline{Haskell}{Void} since the the \mintinline{Haskell}{Op} constructor 
would require a value of type \mintinline{Haskell}{Void'}, which we cannot 
construct.\footnote{It is possible to use \mintinline{Haskell}{undefined} to 
create an impure value of type \mintinline{Haskell}{Prog Void' a}. Since this 
is not possible in Coq, we do not consider this in the Haskell implementation.}
Only \mintinline{Haskell}{Return} can be used to define values, similar to the 
original data type.

Similar to \mintinline{Haskell}{Void'}, we can define a data type 
\mintinline{Haskell}{Choice'} that represents \mintinline{Haskell}{Choice} in 
combination with \mintinline{Haskell}{Prog}. 

\begin{minted}{Haskell}
data Choice' p = Fail' | Choice' p p
\end{minted}

Again, we can omit the \mintinline{Haskell}{Return} constructor because it is 
already part of the \mintinline{Haskell}{Prog} data type. For the same reason, 
the type variable \mintinline{Haskell}{a} has been replaced with the variable 
\mintinline{Haskell}{p}, since \mintinline{Haskell}{Choice} does not have 
...
Since \mintinline{Haskell}{Op} applies \mintinline{Haskell}{sig} recursively, 
this yields the following type, which is equivalent to the original data type 
\mintinline{Haskell}{Choice}.
...
\mintinline{Haskell}{Prog}, which essentially models a tree with leafs, 
represented by the \mintinline{Haskell}{Return} constructor, and branches that 
have the form defined by \mintinline{Haskell}{sig}. \todo{Tree structure 
visualization?}
...
The data type \mintinline{Haskell}{Prog} is better known as the \textit{free 
monad}.
