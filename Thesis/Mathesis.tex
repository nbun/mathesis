...
\subsection{Implementing Scoped Effects}
...
The implementation of the sharing effect presented in \autoref{subsec:sharingImplementaiton} then combines the ideas and advantages of both approaches.

\paragraph{Explicit Scope Syntax}
...
Consequently, the main finding from this definition is that we need to assign IDs to sharing scopes, too, so that we can differentiate between scopes that were created explicitly and scopes that were duplicated by choices in the program structure.
