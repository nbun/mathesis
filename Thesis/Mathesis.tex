...
Verification techniques like testing, static analysis and formal proofs an important part of finding programming errors and ensuring that software works as specified.
...
Verifying the functional language Haskell in Coq, an interactive proof assistant, is an active field of research. \todo{citation needed}
Due to its syntactic resemblance of Haskell, one might assume that Curry can be verified in Coq in a similar matter.
However, some language aspects, that serve a purely performance-related purpose in Haskell, interact with Curry's non-determinism in a way that necessitates explicit consideration in the model.
...
\section{Goals}
The goal of this thesis is to model Curry's call-time choice effect using free monads.
This includes exploring how effects can be modeled in other languages, for example, how the KiCSS2 compiler represents non-determinism when compiling Curry code to Haskell or how an explicit sharing operator can be implemented.
...
\chapter{Curry Programs modeled in Coq}
\begin{itemize}
