...
Inductive Ext
          ...
          (Pos : Shape -> Type)
          (Pos__X : Shape -> Type)
          (Ctx : forall s : Shape, Pos__X s -> Type)
          (F : Type -> Type)
          A :=
  ext : forall s, (Pos s -> F A) -> (forall p : Pos__X s, F (Ctx s p)) -> Ext Shape Pos Pos__X Ctx F A.
...
Class HContainer (H : (Type -> Type) -> Type -> Type) :=
  {
    ...
    Pos__X    : Shape -> Type;
    Ctx     : forall s : Shape, Pos__X s -> Type;
    to      : forall F A, Ext Shape Pos Pos__X Ctx F A -> H F A;
    from    : forall F A, H F A -> Ext Shape Pos Pos__X Ctx F A;
    to_from : forall F A (fx : H F A), to (from fx) = fx;
    from_to : forall F A (e : Ext Shape Pos Pos__X Ctx F A), from (to e) = e
  }.
...
Section Zero.
  ...
  Definition Ctx__Zero (s : Shape__Zero) : Pos__Zero s -> Type := match s with end.
...
(*   Definition Ctx__Comb (s : Shape__Comb) (p : Pos__Comb s) : Type -> Type. *)
(*     destruct s. *)
(*     - apply Ctx with (s0 := s). *)
(*       apply p. *)
(*     - apply Ctx with (s0 := s). *)
...
(*   Definition Ext__Comb := Ext Shape__Comb Pos__Comb Ctx__Comb. *)
...
Section Free.
  ...
  | pure : A -> Free C A
  | impure : Ext Shape Pos Pos__X Ctx (Free C) A -> Free C A.
...
Section MonadInstance.
  ...
  Definition cmap A B F (f : F A -> F B) (x : Ext Shape Pos Pos__X Ctx F A) : Ext Shape Pos Pos__X Ctx F B.
    destruct x.
    apply ext with (s0 := s).
    - intros.
      apply f.
